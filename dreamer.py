...
def make_env(config, logger, mode, train_eps, eval_eps):
    ...
    if suite == "dmc":
        ...
        env = ImageEnvMetaworld(
            make_metaworld_env(task, {}),
            imwidth=64,
            imheight=64,
            reward_scale=config.environment_reward_scale,
        )
...
def process_episode(config, logger, mode, train_eps, eval_eps, episode):
    ...
    logger.scalar(f"{mode}_success", episode["success"][-1])
...
def main(config):
    ...
    if hasattr(acts, "discrete"):
        ...
        random_actor = tfd.Independent(tfd.Uniform(acts.low[None], acts.high[None]), 1)
